create unique index reservation_lifecycle_audit_once_idx
on public.audit_logs (action, target_type, target_id)
where action in ('reservation.completed', 'reservation.no_show')
  and target_type = 'reservation';

create unique index reservation_lifecycle_notification_once_idx
on public.notifications (type, ((data ->> 'reservationId')))
where type in ('reservation.completed', 'reservation.no_show', 'review.requested');

create unique index reservation_coach_no_show_refund_once_idx
on public.refunds (reservation_id, source)
where source = 'manual'
  and reason = 'reservation.coach_no_show';

drop function if exists public.transition_reservation_lifecycle(uuid, public.reservation_status_action, text);
...
declare
  ...
  transition_instant timestamptz := transaction_timestamp();
begin
