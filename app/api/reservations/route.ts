...
import {
  ...
  runCreateReservationWorkflow,
} from "@/lib/reservations/create-reservation-api"
...
export async function POST(request: Request) {
  ...
  const parsedRequest = parseCreateReservationRequest(requestJson.value)

  if (parsedRequest.status === "failure") {
    return apiError("VALIDATION_ERROR", "Invalid reservation request.", 422, parsedRequest.issues)
  }
  ...
  const workflowResult = await runCreateReservationWorkflow(parsedRequest.request, {
    createPendingReservation: async (reservationRequest) => {
      const { data: reservation, error } = await supabase.rpc("create_pending_reservation", {
        checked_lesson_id: reservationRequest.lessonId,
        checked_schedule_id: reservationRequest.lessonScheduleId,
      })
