...
Owner management variant:

- 공개 상태, 별점, 작성일, 레슨명, 본문을 한 행에서 스캔 가능하게 구성한다.
- `hidden`에서만 작성자 전용 숨김 사유를 warning 상태로 표시한다.
- 공개 중인 레슨만 상세 링크를 제공하고, 이력은 레슨 비공개 후에도 안전한 제목으로 남긴다.
- 레슨 링크와 페이지 이동 컨트롤은 최소 44px 높이를 유지한다.
- 새 토큰을 추가하지 않고 기존 semantic token, `StatusBadge`, Lucide 아이콘을 재사용한다.
