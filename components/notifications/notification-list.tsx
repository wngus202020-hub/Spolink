"use client"

import { Check, LoaderCircle, RotateCcw } from "lucide-react"
import { useEffect, useMemo, useRef, useState } from "react"
import { parseRealtimeNotification, readReplicationReadiness } from "@/lib/notifications/realtime"
import { readLatestNotifications } from "@/lib/notifications/realtime-client"
import type { NotificationItem } from "@/lib/notifications/types"
import { createSupabaseBrowserClient } from "@/lib/supabase/client"

export function NotificationList({
  initialItems,
  profileId,
}: Readonly<{ initialItems: readonly NotificationItem[]; profileId: string }>) {
  const supabase = useMemo(() => createSupabaseBrowserClient(), [])
  const [items, setItems] = useState(initialItems)
  const [busyId, setBusyId] = useState<string | null>(null)
  const [errorId, setErrorId] = useState<string | null>(null)
  const [realtimeStatus, setRealtimeStatus] = useState<"connected" | "connecting" | "offline">(
    "connecting",
  )
  const errorRef = useRef<HTMLParagraphElement>(null)

  useEffect(() => {
    if (errorId) errorRef.current?.focus()
  }, [errorId])

  useEffect(() => {
    let active = true
    let channel: ReturnType<typeof supabase.channel> | null = null

    async function connect() {
      await supabase.realtime.setAuth()
      if (!active) return
      channel = supabase
        .channel(`notifications:${profileId}`, {
          config: { broadcast: { replication_ready: true } },
        })
        .on(
          "postgres_changes",
          {
            event: "*",
            filter: `user_id=eq.${profileId}`,
            schema: "public",
            table: "notifications",
          },
          (payload) => {
            const notification = parseRealtimeNotification(payload.new)
            if (!notification) return
            setItems((current) => mergeNotification(current, notification))
          },
        )
        .on("system", {}, (payload) => {
          const readiness = readReplicationReadiness(payload)
          if (readiness === "ready") void synchronize()
          else if (readiness === "error") setRealtimeStatus("offline")
        })
        .subscribe((status) => {
          if (status === "CHANNEL_ERROR" || status === "TIMED_OUT") {
            setRealtimeStatus("offline")
          } else if (status === "CLOSED") setRealtimeStatus("offline")
          else if (status !== "SUBSCRIBED") setRealtimeStatus("connecting")
        })
    }

    async function synchronize() {
      const latest = await readLatestNotifications()
      if (!active) return
      if (latest === null) {
        setRealtimeStatus("offline")
        return
      }
      setItems(latest)
      setRealtimeStatus("connected")
    }

    void connect().catch((error: unknown) => {
      if (!active) return
      if (error instanceof Error) {
        setRealtimeStatus("offline")
        return
      }
      throw error
    })

    return () => {
      active = false
      if (channel) void supabase.removeChannel(channel)
    }
  }, [profileId, supabase])

  async function markRead(id: string) {
    if (items.some((item) => item.id === id && item.readAt !== null)) return
    setBusyId(id)
    try {
      const response = await fetch(`/api/notifications/${id}/read`, {
        body: "{}",
        headers: { "Content-Type": "application/json" },
        method: "POST",
      })
      if (!response.ok) throw new Error("Notification read request failed.")
      const readAt = new Date().toISOString()
      setItems((current) => current.map((item) => (item.id === id ? { ...item, readAt } : item)))
      setErrorId(null)
    } catch (error) {
      if (error instanceof Error) setErrorId(id)
      else throw error
    } finally {
      setBusyId(null)
    }
  }

  return (
    <div className="grid gap-3">
      <p className="m-0 flex items-center gap-2 text-xs font-medium text-secondary" role="status">
        <span
          aria-hidden="true"
          className={`size-2 rounded-full ${realtimeStatus === "connected" ? "bg-[var(--status-success)]" : "bg-[var(--text-tertiary)]"}`}
        />
        {realtimeStatusMessage(realtimeStatus)}
      </p>
      {items.length === 0 ? (
        <p className="m-0 rounded-[var(--radius-lg)] border border-dashed border-line p-8 text-center text-sm text-secondary">
          새 알림이 없어요.
        </p>
      ) : (
        <ul className="m-0 grid list-none gap-3 p-0">
          {items.map((item) => {
            const isRead = item.readAt !== null
            return (
              <li
                className={`grid gap-3 rounded-[var(--radius-lg)] border border-line p-4 ${isRead ? "bg-canvas" : "bg-accent-soft"}`}
                key={item.id}
              >
                {errorId === item.id ? (
                  <p
                    aria-live="assertive"
                    className="m-0 flex items-center justify-between gap-3 rounded-[var(--radius-md)] border border-[color-mix(in_srgb,var(--status-error)_24%,transparent)] bg-[color-mix(in_srgb,var(--status-error)_8%,transparent)] p-3 text-sm text-[var(--status-error)]"
                    id={`notification-error-${item.id}`}
                    ref={errorRef}
                    role="alert"
                    tabIndex={-1}
                  >
                    <span>읽음 처리에 실패했어요. 다시 시도해 주세요.</span>
                    <button
                      aria-label="알림 읽음 처리 다시 시도"
                      className="inline-flex size-11 shrink-0 items-center justify-center rounded-full border border-line bg-canvas text-primary"
                      onClick={() => void markRead(item.id)}
                      type="button"
                    >
                      <RotateCcw aria-hidden="true" className="size-4" />
                    </button>
                  </p>
                ) : null}
                <div className="flex items-start justify-between gap-3">
                  <div className="grid gap-1">
                    <strong className="text-base text-primary">{item.title}</strong>
                    {item.body ? (
                      <p className="m-0 text-sm leading-[1.6] text-secondary">{item.body}</p>
                    ) : null}
                  </div>
                  <button
                    aria-label={isRead ? "읽은 알림" : "알림 읽음 처리"}
                    aria-describedby={
                      errorId === item.id ? `notification-error-${item.id}` : undefined
                    }
                    className="inline-flex size-11 shrink-0 items-center justify-center rounded-full border border-line bg-canvas text-primary disabled:opacity-50"
                    disabled={isRead || busyId === item.id}
                    onClick={() => void markRead(item.id)}
                    type="button"
                  >
                    {busyId === item.id ? (
                      <LoaderCircle aria-hidden="true" className="size-4 animate-spin" />
                    ) : (
                      <Check aria-hidden="true" className="size-4" />
                    )}
                  </button>
                </div>
                <time className="text-xs text-secondary" dateTime={item.createdAt}>
                  {new Intl.DateTimeFormat("ko-KR", {
                    dateStyle: "medium",
                    timeStyle: "short",
                  }).format(new Date(item.createdAt))}
                </time>
              </li>
            )
          })}
        </ul>
      )}
    </div>
  )
}

function mergeNotification(
  current: readonly NotificationItem[],
  incoming: NotificationItem,
): readonly NotificationItem[] {
  const withoutIncoming = current.filter((item) => item.id !== incoming.id)
  return [incoming, ...withoutIncoming]
    .sort(
      (left, right) =>
        right.createdAt.localeCompare(left.createdAt) || right.id.localeCompare(left.id),
    )
    .slice(0, 50)
}

function realtimeStatusMessage(status: "connected" | "connecting" | "offline"): string {
  switch (status) {
    case "connected":
      return "실시간 알림 연결됨"
    case "connecting":
      return "실시간 알림 연결 중"
    case "offline":
      return "실시간 연결이 끊겨 새 알림을 다시 확인해 주세요."
    default:
      return status satisfies never
  }
}
