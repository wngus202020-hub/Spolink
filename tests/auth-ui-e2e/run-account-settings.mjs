...
const outputDir = await mkdtemp(path.join(os.tmpdir(), "spolink-account-settings-"))
...
try {
  await withConfiguredAuthMode({ enableConfirmations: false }, async ({ baseUrl, status }) => {
    ...
    if (exitCode !== 0) {
      throw new Error(`Account settings browser QA failed with exit code ${exitCode}.`)
    }
  })
} finally {
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(
      ...
      [
        ...
        "tests/auth-ui-e2e/account-settings.spec.ts",
    ...
    child.once("exit", (code, signal) => {
      if (signal) {
        reject(new Error(`Account settings browser QA exited from ${signal}.`))
        return
      }
      resolve(code ?? 1)
    })
