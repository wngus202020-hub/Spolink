...
test("direct mutation matrix: reservation writes are workflow-only", async () => {
  try {
...
test("direct mutation matrix: schedule ordinary writes are workflow-only", async () => {
  try {
    for (const persona of ["learner", "otherLearner", "coach", "pendingCoach", "admin"]) {
...
test("direct mutation matrix: schedule reserved_count is workflow-only", async () => {
  try {
    for (const persona of ["coach", "admin"]) {
      await assertScheduleReservedCountCell({
        name: `direct mutation ${persona} reserved_count denied`,
        client: clients[persona],
        allowed: false,
      })
    }
